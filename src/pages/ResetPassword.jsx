...
import { Lock, Loader2, Menu, X, ArrowRight, AlertTriangle, CheckCircle } from "lucide-react";
...
export default function ResetPassword() {
  ...
  const handleSubmit = async (e) => {
    ...
    } catch (err) { setError(err.response?.data?.message || "重置密码失败"); } finally { setLoading(false); }
  };

  const inputCls = "w-full bg-white/[0.06] text-white text-sm placeholder:text-white/30 rounded-xl pl-11 pr-4 py-3 outline-none border border-white/[0.08] focus:border-white/20 transition";
          ...
          {!resetToken ? (
            ...
            <>
              <div className="liquid-glass rounded-full px-5 py-2 mb-5 sm:mb-6">
                <p className="text-xs sm:text-sm text-white/80" style={{ fontFamily: "system-ui, sans-serif" }}>链接无效</p>
              </div>
              ...
              <Link to="/forgot-password" className="mt-6 px-6 py-3 bg-white text-black text-sm font-medium rounded-xl hover:bg-white/90 transition" style={{ fontFamily: "system-ui, sans-serif" }}>
                重新申请
              </Link>
            </>
          ) : (
            ...
            <>
              <div className="liquid-glass rounded-full px-5 py-2 mb-5 sm:mb-6">
                <p className="text-xs sm:text-sm text-white/80" style={{ fontFamily: "system-ui, sans-serif" }}>设置新密码</p>
              </div>
              <h1 className={`text-center text-4xl sm:text-5xl md:text-7xl lg:text-[5.5rem] leading-[1.1] max-w-4xl font-normal transition-colors duration-700 ${isDark ? "text-[#182C41]" : "text-white"}`} style={{ fontFamily: "'Instrument Serif', serif" }}>
              ...
              <div className="mt-6 sm:mt-8 liquid-glass rounded-3xl p-6 sm:p-8 w-full max-w-sm space-y-4">
                <div className="relative">
                  <Lock className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-white/40" />
                  <input type="password" placeholder="新密码" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} required className={inputCls} style={{ fontFamily: "system-ui, sans-serif" }} />
                </div>
                <div className="relative">
                  <Lock className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-white/40" />
                  <input type="password" placeholder="确认新密码" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} required className={inputCls} style={{ fontFamily: "system-ui, sans-serif" }} />
                </div>
                <AnimatePresence>
                  {error && (
                    <motion.p initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="text-sm text-red-300 text-center" style={{ fontFamily: "system-ui, sans-serif" }}>{error}</motion.p>
                  )}
                </AnimatePresence>
                <button type="submit" disabled={loading} onClick={handleSubmit} className="w-full py-3 bg-white text-black text-sm font-medium rounded-xl hover:bg-white/90 transition flex items-center justify-center gap-2 disabled:opacity-50" style={{ fontFamily: "system-ui, sans-serif" }}>
                  {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : <>重置密码 <ArrowRight className="w-4 h-4" /></>}
                </button>
              </div>
